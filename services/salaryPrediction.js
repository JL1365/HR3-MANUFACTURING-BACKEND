...
const fetchPayrollData = async () => {
    try {
        ...
        const groupedRecords = records.reduce((acc, record) => {
            ...
            acc[record.employee_id].totalAdjustedSalary += record.adjustedSalary;
...
const prepareData = (records) => {
  ...
  const grossSalaries = records.map(record => record.averageSalary);

  return { inputs, grossSalaries };
};

const trainModel = async (inputData, grossSalaryData) => {
  ...
  const outputTensor = tf.tensor2d(grossSalaryData, [grossSalaryData.length, 1]);
...
const predictSalaries = async () => {
  ...
  const predictedData = records.map((record, index) => {
    const predictionId = `${record.employee_id}-${record.payroll_date.toISOString()}`;
